...
function aiChoose() {
    const aryaman = Math.floor(Math.random() * CHOICES.length);
    return CHOICES[aryaman];
}

function displayResults(results) {
    resultDivs.forEach((resultDiv, id) => {
        setTimeout(() => {
            resultDiv.innerHTML = `
          <div class="choice ${results[id].name}">
            <img src="images/icon-${results[id].name}.svg" alt="${results[id].name}" />
          </div>
        `;
        }, id * 1000);
    });
